...
environment = os.environ['irsch_env'] if os.environ['irsch_env'] else "dev" #Defaults to dev if not environment is specified
...
def lambda_handler(event, context):
	...
	resp = get_secret("irsch_"+environment+"_sftp/" + input_username)
...
def get_secret(id):
	region = os.environ['SecretsManagerRegion'] if os.environ['SecretsManagerRegion'] else "us-east-1" # this is not working in the new lambda environment
